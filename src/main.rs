...
impl Giza {
    ...
    fn create_metal_state() -> anyhow::Result<MetalState> {
        ...
        let pipeline_state = Self::create_pipeline_state(device.as_ref())
    ...
    fn draw(&mut self) -> anyhow::Result<()> {
        ...
        command_encoder.setRenderPipelineState(metal_state.pipeline_state.as_ref());

        // Safety: This draw call only produces vertex IDs 0..=2, which matches the
        // three fixed vertices generated by the current Metal vertex shader.
        unsafe {
    ...
    ) -> anyhow::Result<Retained<ProtocolObject<dyn MTLRenderPipelineState>>> {
        let shader_library = device
            .newLibraryWithSource_options_error(ns_string!(include_str!("shaders.metal")), None)
            .context("failed to compile Metal shader library")?;
        let vertex_function = shader_library
            .newFunctionWithName(ns_string!("vertex_main"))
            .context("failed to find vertex_main in shader library")?;
        let fragment_function = shader_library
            .newFunctionWithName(ns_string!("fragment_main"))
            .context("failed to find fragment_main in shader library")?;

        let pipeline_descriptor = MTLRenderPipelineDescriptor::new();
        pipeline_descriptor.setVertexFunction(Some(vertex_function.as_ref()));
        pipeline_descriptor.setFragmentFunction(Some(fragment_function.as_ref()));

        let color_attachment =
            // Safety: `colorAttachments[0]` is the first valid color attachment slot in Metal.
            unsafe { pipeline_descriptor.colorAttachments().objectAtIndexedSubscript(0) };
        color_attachment.setPixelFormat(MTLPixelFormat::BGRA8Unorm);

        let pipeline_state = device
            .newRenderPipelineStateWithDescriptor_error(&pipeline_descriptor)
            .context("failed to create render pipeline state")?;
